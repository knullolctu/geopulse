...
const navItems = [
  { label: 'My Dashboard', to: '/attendee/dashboard', icon: '🏠' },
  { label: 'Attendance History', to: '/attendee/logs', icon: '📜' },
]
...
export default function AttendeeSidebar({ isOpen, onClose }: AttendeeSidebarProps) {
  return (
    <>
      {/* Mobile Backdrop */}
      {isOpen && (
      ...
      <aside
        ...
        `}
        style={{ background: '#fef2f2', borderRight: `4px solid ${THEME.colors.darkBorder}` }}
      >
        <div className="p-6 flex items-center justify-between">
          <Link to="/attendee/dashboard" className="flex items-center gap-3 group">
            <div
        ...
        <nav className="flex-1 px-4 py-4 space-y-3 overflow-y-auto">
          <div className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] px-3 mb-2">Attendee Portal</div>
          {navItems.map((item) => (
            <Link
              key={item.to}
              to={item.to}
